...
int main(int argc, char *argv[])
{
	...
	if (argc >= 2)
		sound = FALSE; /*optional sound effects*/
#endif
...
/*EMPTY. it is responsible for updating the current line number and    */
{
	int i;
	int incr, i_incr;
	int top_lin, bot_lin;
	...
	{
		if (ptr->col[LEFT] != EMPTY)
		{													  /*there is an invader there*/
...
/* -moving invader         missiles and checking for kills     */
{
	...
	for (i = 0; i < MAX_MISSILES; i++)
	{
		...
		if (screen[*lin][*col] == EXP_CHAR)
		{ /*a splat to be deleted*/
			screen[*lin][*col] = ' ';
...
first_time:
	...
	if (isblank(screen[gun_shot.lin][gun_shot.col]))
	{ /*check for no hits*/
...
/*writes score to display*/
{
	...
	if (score != old_score || no_of_guns != old_guns)
	{
		...
		if (score > GOOD_SCORE)
			mmdelay = DELAY - 1; /*speed up missiles*/
	}
}
...
label0:
	...
	case 0: /*shoot from first row, front*/
		for (l = 0; l < INV_ROWS; l++)
			if (alien[l].no_inv > 0)
				break; /*find bottom row*/
		for (c = 0; c < INV_COLS; c++)
		{
			p = &alien[l].node[c];
			if (p->col[LEFT] != EMPTY)
				break; /*first from left*/
		}
	...
	{
		char s;
		for (l = GUN_LIN - 1; l >= 0; l--)
		{ /*start at gun*/
			s = screen[l][gun_col + skew];
			if (!isblank(s) && (s != BLOCK_CHAR) && (s != SHOT_CHAR) && (s != MIS_CHAR) && (s != EXP_CHAR))
			{
				pr->lin = l + 1;
				pr->col = gun_col + skew;
				return;
